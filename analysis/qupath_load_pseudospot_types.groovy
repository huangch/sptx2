...
def FILENAME = "/workspace/sptx2/analysis/predicted_with_membrane_pseudospot_types.csv"
...
while ((row = csvReader.readLine()) != null) {
    ...
    // annotation.get(0).setPathClass(getPathClass(String.valueOf(clusterId)))
     ...
     def clst = getPathClass('spot_type-'+Integer.toString(clusterId))
     annotationsById[spotId][0].setPathClass(clst)
}

rest_annots = annotations.findAll {it.getDisplayedName().startsWith("pseudo-spot")}
rest_annots.each{ p -> p.setPathClass(getPathClass('spot_type-none'))}
...
def detections = annotations.collect {
    ...
    def clsName = d.getPathClass().getName()
    def clst = getPathClass('detection_'+clsName)
    d.setPathClass(clst)
    d
}
