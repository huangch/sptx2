...
def train(opt):
    ...
    if opt.reference_file != "":
        ref_anndata = sc.read_h5ad(opt.reference_file)
        ref_data = ref_anndata.to_df()
        ref_data.dropna(axis=1, how='all', inplace=True) # Remove columns of all NaN
        ref_data.dropna(inplace=True) # Drop rows containing NaN
    
        trns_data = trns_data[[a for a in trns_data.columns if a in ref_data.columns]]
        ref_data = ref_data[[a for a in ref_data.columns if a in trns_data.columns]]
        
        ref_data = np.expm1(ref_data)
        ref_count_per_cell = ref_data.sample(100).sum(axis=1).mean()
        assert (ref_data.sample(100).sum(axis=1)/ref_count_per_cell).std() < 1.0, "reference data is not normalized"
        
        ref_data = ref_data.drop(ref_data[ref_data.sum(axis=1)==0].index)
    ...
    training_transformed_dataset = SingleCellImageDataset(
        ...
        image_std=image_std) # , ref_data=ref_data, trns_ref_index=trns_ref_index)
    ...
    if opt.reference_file != "": ref_data = FloatTensor(ref_data.to_numpy())
    last_gen_cls_acc = 0
    ...
    for epoch in optimizationRange:
        ...
        for i, (imgs, uuid, trns, sbtps) in enumerate(trainingDataLoader):
            ...
            if opt.reference_file != "": 
                dist = torch.cdist(gen_trns, ref_data, p=2, compute_mode='use_mm_for_euclid_dist')
                dist_index = dist.min(dim=1)[1]
                tgt_trns = ref_data[dist_index]
                # real_trns = (real_trns + tgt_trns) / 2
            
            # Loss measures generator's ability to fool the discriminator
            if opt.reference_file != "":
                validity, _, _ = discriminator(tgt_trns)
            else:
            ...
            recon_real_loss = reconstruction_loss(gen_trns, real_trns)
            
            if opt.reference_file != "": 
                recon_tgt_loss = reconstruction_loss(gen_trns, tgt_trns)
                recon_loss = (recon_real_loss + recon_tgt_loss) / 2
            else:
                recon_loss = recon_real_loss
            
            if opt.model == 'vae': kl_loss = generator.kl
            elif opt.model == 'vqvae' or opt.model == 'resvqvae': vq_loss = generator.vq_loss
                
            cell_corr_gen_loss = cell_correlation_loss(gen_trns, real_trns)
            
            if opt.reference_file != "": 
                cell_corr_tgt_loss = cell_correlation_loss(tgt_trns, real_trns)
                cell_corr_loss = (cell_corr_gen_loss+cell_corr_tgt_loss) / 2
            else:
                cell_corr_loss = cell_corr_gen_loss
            
            gene_corr_gen_loss = gene_correlation_loss(gen_trns, real_trns)
            
            if opt.reference_file != "": 
                gene_corr_tgt_loss = gene_correlation_loss(tgt_trns, real_trns)
                gene_corr_loss = (gene_corr_gen_loss+gene_corr_tgt_loss) / 2
            else:
                gene_corr_loss = gene_corr_gen_loss
            ...
            if opt.reference_file != "": 
                fake_pred, fake_aux, gen_cls = discriminator(tgt_trns.detach())
            else:
                fake_pred, fake_aux, gen_cls = discriminator(gen_trns.detach())
            ...
            if opt.tensorboard:
                ...
                tensorboard_writer.add_scalar("training_loss/recon_real_loss", recon_real_loss.item(), epoch)
                if opt.reference_file != "": tensorboard_writer.add_scalar("training_loss/recon_tgt_loss", recon_tgt_loss.item(), epoch)
        ...
        for i, (imgs, uuid, trns, sbtps) in enumerate(validationDataLoader):
            ...
            if opt.tensorboard:
                ...
                tensorboard_writer.add_scalar("validation_loss/recon_real_loss", recon_real_loss.item(), epoch)
                if opt.reference_file != "": tensorboard_writer.add_scalar("validation_loss/recon_tgt_loss", recon_tgt_loss.item(), epoch)
        ...
        if opt.test_interval != 0 and (epoch+1) % opt.test_interval == 0:
            ...
            with open(os.path.join(opt.output_folder, opt.model_prefix+validationCsvFilename), "w", newline='') as f:
                ...
                csv_writer.writerow(['Object ID','subtype']+trns_data.columns.tolist())
                ...
                for i, (imgs, uuid, trns, sbtps) in enumerate(validationDataLoader):
                    ...
                    for j in range(batch_size):
                        row = [uuid[j], gen_pred[j].astype(np.int32)]+gen_trns.data.cpu().numpy()[j].tolist()
    ...
    if opt.final_test:
        ...
        # Create datasets
        total_transformed_dataset = SingleCellImageDataset(total_image_data, total_trns_data, total_subtype_data, image_fileext=existing_image_fileext, image_size=opt.image_size, image_mean=image_mean, image_std=image_std)# , ref_data=ref_data, trns_ref_index=trns_ref_index)
        ...
        with open(os.path.join(opt.output_folder, opt.model_prefix+total_csv_filename), "w", newline='') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(['Object ID','subtype']+param_json_data['gene_id'])
            
            # for i, (imgs, uuid, trns, sbtps) in enumerate(tqdm(total_data_loader)):
            for i, (imgs, uuid, trns, sbtps) in enumerate(total_data_loader):
                batch_size = imgs.shape[0]
                ...
                for j in range(batch_size):
                    row = [uuid[j], gen_pred[j].astype(np.int32)]+gen_trns[j].tolist()
                    # row = [uuid[j]]+gen_trns[j].tolist()
                    csv_writer.writerow(row)
